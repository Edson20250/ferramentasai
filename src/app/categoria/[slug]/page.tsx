            ...
            <div className="flex-1 min-w-0">
              <div className="flex flex-wrap items-center gap-2 mb-1">
                <h1
                  ...
                  style={{ fontSize: 'clamp(1.25rem, 3vw, 1.75rem)', letterSpacing: '-0.025em' }}
                >
                  {categoria.nome}
                </h1>
                <span
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-wrap gap-3 items-center mb-6 pb-6" style={{ borderBottom: '1px solid var(--border)' }}>
              <div className="flex items-center gap-1.5 flex-wrap">
                <span className="text-xs font-semibold text-[var(--muted-foreground)] mr-1">Preço:</span>
                {FILTROS_PRECO.map(f => (
                  <Link key={f.value} href={`/categoria/${slug}?preco=${f.value}&ordem=${ordem}`} className={pill(preco === f.value)}>
                    {f.label}
                  </Link>
                ))}
              </div>
              <div className="flex items-center gap-1.5 ml-auto flex-wrap">
                <span className="text-xs font-semibold text-[var(--muted-foreground)] mr-1">Ordenar:</span>
                {FILTROS_ORDEM.map(f => (
                  <Link key={f.value} href={`/categoria/${slug}?preco=${preco}&ordem=${f.value}`} className={pill(ordem === f.value)}>
                    {f.label}
                  </Link>
                ))}
              </div>
            ...
            {ferramentas.length === 0 ? (
              <EmptyState
                ...
                description="Tenta remover os filtros ou explora outra categoria."
                actions={[
                  { label: 'Ver todas as categorias', href: '/categorias', variant: 'outline' },
                ]}
              />
            ) : (
              <>
