      ...
      <div className="flex gap-8">
        <aside className="hidden lg:block w-52 shrink-0">
          <ul className="space-y-0.5">
            {todasCategorias.map(cat => (
              <li key={cat.id}>
                <Link href={`/categoria/${cat.slug}`} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm ${cat.slug === slug ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {FILTROS_PRECO.map(f => (
              <Link key={f.value} href={`/categoria/${slug}?preco=${f.value}&ordem=${ordem}`} className={`text-xs px-3 py-1 rounded-full border ${preco === f.value ? 'bg-slate-900 text-white' : 'border-slate-200 text-slate-600'}`}>{f.label}</Link>
            ))}
          </div>
